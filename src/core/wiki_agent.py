...
class WikiAgent:
    ...
    def search(self, query: str, on_log: Optional[Callable[[str], None]] = None) -> str:
        ...
        system_prompt = (
            ...
            "【决策准则】：\n"
            "- 优先检索：先用 wiki_search 查找。如果看到【Wiki 聚合页】，请务必优先通过 wiki_read 读取它。一个聚合页往往包含了该实体的全部相关事实，读它即可直接完成任务。\n"
            "- 只有在 Wiki 库中无匹配项时，才尝试深挖原始文档碎片。\n"
            "- 汇总结果必须高度整理，禁止简单堆砌原文。\n"
            "- 必须标注知识来源路径。如果来自 Wiki 聚合页，请标注 [Source: Wiki:名称]。\n"
            "- 最多执行 10 步，尽快得出结论。\n\n"
            "【输出格式 - JSON】：\n"
